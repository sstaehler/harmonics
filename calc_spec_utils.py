...
def calc_spec(fnam_smgr, fmin=1e-2, fmax=10, vmin=-180, vmax=-80, winlen=300,
              pick_harmonics=True, fmin_pick=0.4, fmax_pick=1.2, s_threshold=-140,
              path_out='.'):
    ...
    if (pick_harmonics):
        times, freqs, idxs = harmonics.pick_spectrogram(f, t, slog,
                                                        fwin=(fmin_pick, fmax_pick),
                                                        s_threshold=s_threshold)
        for time, freq, idx in zip(times, freqs, idxs):
            posx = time - winlen/4
            posy = fmax_pick
